...
namespace Proyecto
{
    class ProductosMasVendidos: Conexion
    {
        ...
        public DataTable getMejoresVentas(DateTime fromDate, DateTime toDate,int rango)
        {
            using (var connection = GetConnection())
            {
                ...
                using (var command = new MySqlCommand())
                {
                    ...
                    command.Parameters.Add("@startDate", MySqlDbType.Date).Value = fromDate;
                    command.Parameters.Add("@endDate", MySqlDbType.Date).Value = toDate;
        ...
        public DataTable getPeoresVentas(DateTime fromDate, DateTime toDate, int rango)
        {
            using (var connection = GetConnection())
            {
                connection.Open();
                using (var command = new MySqlCommand())
                {
                    ...
                    where o.fecha_orden between @startDate and @endDate
                    GROUP by p.id_Articulo,p.nombre_articulo,p.precio_Articulo
                    ORDER by cantidadvendida ASC LIMIT 0,10
                    ";
                    command.CommandType = CommandType.Text;
                    command.Parameters.Add("@startDate", MySqlDbType.Date).Value = fromDate;
                    command.Parameters.Add("@endDate", MySqlDbType.Date).Value = toDate;
